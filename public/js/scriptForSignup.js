...
$(function(){
  ...
  $('#uname').keyup(function(){
    ...
    if(uname.length < 5){
      ...
      $('#ik').hide();
      $('#ir').show();
      $('#error1').show().text("Username must contain min 5 characters.");
    }
    else{
    ...
    if (atpos < 1 || dotpos < atpos+2 || dotpos+2 >= email.length){
      ...
      socket.on('checkEmail',function(data){
        if(data == 1){
          ...
          $('#ik1').hide();
          $('#ir1').show();
          $('#error1').show().text("Email Already Exists");
        }
      });
    }
  ...
  $('#pass').keyup(function(){
    ...
    if(pass.length < 5){
      ...
      $('#ik2').hide();
      $('#ir2').show();
      $('#error1').show().text("Password Should be min 5 characters.");
    }
    else{
      pflag = 1;
      $('#ik2').show();
  ...
  $('form').submit(function(){
    ...
    if(uflag == 1 && eflag == 1 && pflag == 1){
      ...
      $('#error1').show().text("Incorrect input");
      return false;
    }
  });
